...
namespace WikiFeet
{
    ...
    /// </summary>
    public class FeetOfTheWeek
    {
        private readonly string _modelInfo;
        private const string PatternModelInfo = "Feet of the week(.*?)<a href='/(.*?)'><div>";
        ...
        private async Task<string> ModelInfo()
        {
            try
            {
                string url = await Http("https://www.wikifeet.com");
                ...
                if (url != null)
                {
                    Match matcher = regex.Match(url);
                        if (matcher.Success)
                        {
                            string modelName = new Regex("_| ").Replace(matcher.Groups[2].Value, " ");
